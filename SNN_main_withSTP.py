...
trained_data_path='TrainedData/mnist-digit/'              # specify the location of trained data(weight, theta,assignment, spike count for exc neurons per image sample and performance)
...
testSamples=200
Epochs=1
#since relabling phase is active after training phase which synaptic weights
#are fixed, relabling phase will be applied on whole training set with 800 without any epochs
if relabling:
    trainSamples=800
else:
    trainSamples=800*Epochs+1
...
###########################################################################
''' In Pipeline 2, you need to first train on the entire training dataset 
using the file SNN_main_noSTP.py.Next, change TestMode and relabeling to True 
in the file SNN_main_withSTP. After that, run evaluation_relabeling to generate 
assignments for the excitatory neurons. Finally, execute the file SNN_withSTP in testing mode.
and change relabling to False '''

#chenge relabling  to True when running relabling phase in pipeline 2:
#relabling=True
if relabling:
    testing=training
else:
    testing=testing
print("relabling", relabling)
#------------------------------------------------------------------------------
...
#eqs_stdp_pre_ee = 'pre = 1.; w = clip(w +nu_ee_pre * post1+0.01*w*r_S, 0, wmax_ee)'
#eqs_stdp_post_ee = 'post2before = post2; w = clip(w +nu_ee_post * pre * post2before, 0, wmax_ee); post1 = 1.; post2 = 1.'
...
#Adding STP parameters
### ##########################Synapse parameters###################################
Omega_f = 3.33/second        # Synaptic facilitation rate
Omega_d = 2.0/second         # Synaptic depression rate
###############################external parameters#################################
k_r_S=10.5                   #degree factor
U_0=0.6
###############################Synapse equations for STP#########################
synapses_eqs = '''
...
for subgroup_n, name in enumerate(population_names):    #'A'
    ...
    spike_counters[name+'i'] = b2.SpikeMonitor(neuron_groups[name+'i'])
    spike_monitors[name+'e'] = b2.SpikeMonitor(neuron_groups[name+'e'])
#------------------------------------------------------------------------------
# create input population and connections from input populations
#------------------------------------------------------------------------------
for name in input_population_names:                                #input_population_names=X
...
for name in input_connection_names:                               #input_connection_names='XA'
    ...
    for connType in input_conn_names:   #input_conn_names='ee_input'
        ...
        model =synapses_eqs+ 'w : 1'
        #pre = 'g%s_post += w' % connType[0]
        delta='w*r_S + w'
        print("delta", delta)
        ##add STP effect
        pre = synapses_action+'g%s_post += (k_r_S*w*r_S + w)'% connType[0]
        #print(g%s_post % connType[0])
        post = ''
        if ee_STDP_on:
        ...
        connections[connName] = b2.Synapses(input_groups[connName[0:2]], neuron_groups[connName[2:4]],
                                                    model=model, on_pre=pre, on_post=post, name="input_synapses")
        #delay['ee_input'] = (0*b2.ms,10*b2.ms)
        ...
        connections[connName].w = weightMatrix[connections[connName].i, connections[connName].j]
        #add x_s
        connections[connName].x_S = 1.0
#------------------------------------------------------------------------------
# run the simulation and set inputs
#------------------------------------------------------------------------------
#store all spikes individually per input for each exc neuron
start = time.time()
...
while j < (int(num_examples)):
    ...
    if j % update_interval == 0 and j > 0:  #testmode=num_example
        assignments = myfunctions.get_new_assignments(result_monitor[:], input_numbers[j-update_interval : j])
        #print("j="+j+" passed update_interval")
        #np.save(data_path + 'assignment/assignment' + str(j), assignments)
        #np.save(data_path + 'assignment/resultPopVecs' + str(j), result_monitor)
        
        #np.save(data_path+ 'assignment/spikeMonitors_Ae_t'+str(j),spike_monitors['Ae'].t)
        #np.save(data_path+ 'assignment/spikeMonitors_Ae_i'+str(j),spike_monitors['Ae'].i)
        #np.save(data_path+ 'assignment/rate_monitors_Ae_rate'+str(j),rate_monitors['Ae'].rate)
        #np.save(data_path+ 'assignment/rate_monitors_Ae_t'+str(j),rate_monitors['Ae'].t)
        #np.save(data_path+ 'assignment/rate_monitors_Ai_rate'+str(j),rate_monitors['Ai'].rate)
        #np.save(data_path+ 'assignment/rate_monitors_Ai_t'+str(j),rate_monitors['Ai'].t)
        #np.save(data_path+ 'assignment/rate_monitors_Xe_rate'+str(j),rate_monitors['Xe'].rate)
        #np.save(data_path+ 'assignment/rate_monitors_Xe_t'+str(j),rate_monitors['Xe'].t)
        #np.save(data_path+ 'assignment/spikeMonitors_Ae_count'+str(j),spike_monitors['Ae'].count[:])
    ...
    if np.sum(current_spike_count) < 5:
        ...
        if j % update_interval == 0 and j > 1:
            if do_plot_performance:
                print("performanceshape",performance)
                unused, performance = update_performance_plot(performance_monitor, performance, j, fig_performance)
                print('Classification performance', performance[:int(j/float(update_interval))+1])
                #np.save(data_path + 'assignment/performance' + str(j), performance)
...
if not test_mode:
    ...
    #print(assignmnets)
    np.save(data_path + 'assignment/resultPopVecs' + str(num_examples), result_monitor)
    np.save(data_path + 'assignment/assignment' + str(j), assignments)
    #np.save(data_path + 'activity/performance' + str(k), performance)
    
    #np.save(data_path + 'activity/inputNumbers' + str(num_examples), input_numbers)
else:
    if relabling:
        np.save(data_path + 'activity/resultPopVecs' + str(num_examples)+'STPrelabling', result_monitor)
        np.save(data_path + 'activity/inputNumbers' + str(num_examples)+ 'STPrelabling' , input_numbers)
        np.save(data_path + 'activity/spike_info' + str(num_examples)+'STPrelabling', spike_info)
      
    else:
        np.save(data_path + 'activity/resultPopVecs' + str(num_examples)+'STP', result_monitor)
        np.save(data_path + 'activity/inputNumbers' + str(num_examples)+ 'STP' , input_numbers)
        np.save(data_path + 'activity/spike_info' + str(num_examples)+'STP', spike_info)
